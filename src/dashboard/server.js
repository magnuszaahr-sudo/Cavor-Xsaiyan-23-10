/**
 * Cavor-Xv3-11/x — Dashboard Server (Express + Socket.io)
...
function _addBotMsg(threadID, body, attachments) {
  ...
  const msgObj = {
    ...
    body: body || "",
    senderID: botID,
    senderName: global.GoatBot?.config?.botName || "Cavor-Xv3-11/x",
    ts: Date.now(),
...
function getDashPwd() {
  try {
    ...
    return cfg.dashboard?.password || "cavor2025";
...
function getStats() {
  ...
  return {
    ...
    botName:       global.GoatBot?.config?.botName || "Cavor-Xv3-11/x",
...
function auth(req, res, next) {
  const tok = (req.headers["x-cavor-token"] || req.headers["x-cavor-token"]) || req.query.token;
...
function startDashboard(port = 5000) {
  ...
  app.get("/api/status", auth, (_, res) => {
    ...
    res.json({ ok: true, online, botID: global.GoatBot?.botID || null, botName: global.GoatBot?.config?.botName || "Cavor-Xv3-11/x" });
  });
  ...
  app.post("/api/cookies", auth, (req, res) => {
    ...
    try {
      ...
      const parsed    = CavorFCA.parseCookieInput(raw);
      const cookies   = parsed.cookies;
      if (!cookies.length) return res.json({ ok: false, error: "صيغة الكوكيز غير معروفة" });
      if (!CavorFCA.hasMandatory(cookies)) return res.json({ ok: false, error: "كوكيز ناقصة (c_user أو xs مفقود)" });
  ...
  app.post("/api/messenger/send-with-caption", auth, async (req, res) => {
    ...
    const tmpPath = path.join(os.tmpdir(), `cavor-${Date.now()}-${safeName}`);
    try {
  ...
  app.post("/api/messenger/set-image", auth, async (req, res) => {
    ...
    if (!threadID || !data) return res.json({ ok: false, error: "threadID و data مطلوبان" });
    const tmpPath = path.join(os.tmpdir(), `cavor-img-${Date.now()}.jpg`);
    try {
      fs.writeFileSync(tmpPath, Buffer.from(data, "base64"));
      const stream = fs.createReadStream(tmpPath);
  ...
  app.post("/api/ai/generate", auth, async (req, res) => {
    try {
      ...
      const systemPrompt = `أنت مطور بوت فيسبوك ماسنجر خبير بـ Cavor-Xv3-11/x Engine. مهمتك كتابة أوامر Node.js للبوت.
...
مثال حقيقي:\n${exampleContent}\n${contextContent ? `\nسياق إضافي:${contextContent}` : ""}`;
  ...
  app.post("/api/ai/chat", auth, async (req, res) => {
    try {
      ...
      const result = await _callAI(
        "أنت مساعد ذكي خبير في بوتات فيسبوك ماسنجر ونظام Cavor-Xv3-11/x. أجب باختصار ووضوح باللغة العربية.",
  ...
  app.get("/api/github/repo-info", auth, async (req, res) => {
    try {
      ...
      const headers = { "User-Agent": "Cavor-Xv3-11-x-Bot", Accept: "application/vnd.github.v3+json" };
  ...
  app.post("/api/github/push", auth, async (req, res) => {
    ...
    try {
      const { token, repo = "castrolmocro/divid-apk", scope = "bot", message = "🤖 Cavor-Xv3-11/x - Auto Update" } = req.body;
      if (!token) return res.json({ ok: false, error: "GitHub Token مطلوب", log });
      const ghHeaders = {
        "Authorization": `token ${token}`,
        "User-Agent": "Cavor-Xv3-11-x-Bot",
      ...
      if (scope === "bot") {
        ["src","Cavor.js","index.js","package.json","config.json","manifest.json"].forEach(f => {
