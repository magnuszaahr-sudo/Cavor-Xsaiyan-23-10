/**
 * Cavor-Xv3-11/x — MQTT Health Check (Layer 6)
...
async function doCheck() {
  ...
  if (cfg.notify) {
    try {
      ...
      if (api) for (const id of ids) { try { api.sendMessage(`⚠️ [Cavor-Xv3-11/x] إعادة الاتصال التلقائية (#${_count})`, String(id)).catch(()=>{}); } catch(_){} }
    } catch (_) {}
  }
