/**
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║   Cavor-FCA — Media Helpers v1.0                                  ║
 * ║   إرسال الصور والفيديوهات والصوت من URL مباشرة                    ║
 * ║   Copyright © 2025 CAVOR — Cavor-Xv3-11/x                               ║
 * ╚══════════════════════════════════════════════════════════════════════╝
 *
 * الاستخدام في الأوامر:
 *   const { sendPhoto, sendVideo, sendAudio, sendFile, cobaltDownload } = require("../../Cavor-fca/lib/media");
 *
 ...
 *
 * أو عبر api.cavor مباشرة:
 *   await api.cavor.sendPhoto(url, caption, threadID);
 */
"use strict";
...
const TMP = path.join(os.tmpdir(), "cavor_media");
...
async function downloadStream(url, options = {}) {
  ...
  res.data.path = `cavor_media_${Date.now()}.${ext}`;
  return res.data;
}

// ── تنزيل إلى ملف مؤقت ───────────────────────────────────────────────────────
async function downloadToFile(url, ext, options = {}) {
  const outPath = path.join(TMP, `cavor_${Date.now()}.${ext}`);
  const res     = await axios.get(url, {
...
async function sendMultiPhoto(api, urls, caption, threadID, opts = {}) {
  ...
  if (!valid.length) throw new Error("cavor.media: لم يتم تنزيل أي صورة");
  return api.sendMessage(
