/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║           Cavor-Xv3-11/x — CavorBot Engine (المحرك الرئيسي)         ║
 * ║           Copyright © 2025 CAVOR — All rights reserved        ║
 * ║           Version 2.0 | Engine: CAVOR                          ║
 * ╚══════════════════════════════════════════════════════════════════╝
 */
"use strict";
...
process.on("unhandledRejection", e => { try { (global.log?.error || console.error)("CAVOR", e?.message || String(e)); } catch (_) {} });
process.on("uncaughtException",  e => { try { (global.log?.error || console.error)("CAVOR", e?.message || String(e)); } catch (_) {} });
...
╚═════╝ ╚═╝  ╚═╝  ╚═══╝  ╚═╝╚═════╝       ╚═══╝   ╚═╝`;
  ...
  console.log(chalk.hex("#ffd166")(`  Developer  : CAVOR`));
  console.log(chalk.hex("#06d6a0")(`  Engine     : CAVOR v2.0`));
  console.log(chalk.hex("#90e0ef")(`  Library    : Cavor-fca v3.0`));
...
async function startBot() {
  ...
  const { parseCookieInput, hasMandatory, checkLiveCookie, cookiesToString, getUA } = CavorFCA;
  ...
  function tryLogin() {
    ...
    CavorFCA(cookies, { userAgent: UA }, async (err, api, extras) => {
      if (err) {
      ...
      // اسم البوت
      let botName = config.botName || "Cavor-Xv3-11/x";
      try {
      ...
      console.log(`  │  ${chalk.yellow("Engine:")}   ${chalk.white("CAVOR v2 — 20 Protection Layers".padEnd(35))}│`);
      console.log(`  │  ${chalk.yellow("Port:")}     ${chalk.white(String(PORT).padEnd(35))}│`);
      console.log(`  │  ${chalk.yellow("By:")}       ${chalk.white("CAVOR".padEnd(35))}│`);
      console.log(chalk.hex("#00b4d8")("  └──────────────────────────────────────────┘"));
      console.log();
